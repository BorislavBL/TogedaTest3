//
//  InstagramProfileOverlay.swift
//  TogedaTest3
//
//  Created by Borislav Lorinkov on 4.12.24.
//

import SwiftUI

struct InstagramProfileOverlay: View {
    ...
    var body: some View {
        ZStack{
            ...
            let content = ProfileInstagramStoryView(user: user)
            ...
            VStack(spacing: 20){
                if step == 1 {
                    ...
                    Button{
                        UIPasteboard.general.string = createURLLink(postID: nil, clubID: nil, userID: user.id)
                        withAnimation {
                            step = 2
                        }
                    } label:{
...
#Preview {
    InstagramProfileOverlay(isActive: .constant(false), user: MockUser)
}
